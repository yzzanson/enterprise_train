...
public class DingTalkEncryptor {
    ...
    public String getDecryptMsgNew(String msgSignature, String timeStamp, String nonce, String encryptMsg) throws DingTalkEncryptException {
        //校验签名
        String signature = getSignature(token, timeStamp, nonce, encryptMsg);
        if (!signature.equals(msgSignature)) {
            throw new DingTalkEncryptException(DingTalkEncryptException.COMPUTE_SIGNATURE_ERROR);
        ...
        // 解密
        String result = decryptNew(encryptMsg);
        return result;
    }
    ...
    private String decrypt(String text) throws DingTalkEncryptException {
        ...
        if (!fromCorpid.equals(corpId)) {
            System.out.println("Corpid1> " + fromCorpid + " corpId >" + corpId);
            throw new DingTalkEncryptException(DingTalkEncryptException.COMPUTE_DECRYPT_TEXT_CORPID_ERROR);
        }
        return plainText;
    }

    private String decryptNew(String text) throws DingTalkEncryptException {
        byte[] originalArr;
        try {
            // 设置解密模式为AES的CBC模式
            Cipher cipher = Cipher.getInstance("AES/CBC/NoPadding");
